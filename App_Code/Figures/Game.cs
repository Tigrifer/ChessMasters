...
public class Game : GameState
{
    ...
    public Color GetColorTurn() { return this.moveCount % 2 == 0 ? Color.white : Color.black; }
    public Game()
    {
        for (sbyte i = 1; i <= 8; i++)
        {
            Pow p1 = new Pow(this, i, 7, Color.black);
            this.blacks.Add(p1);

            Pow wp1 = new Pow(this, i, 2, Color.white);
            this.whites.Add(wp1);
        }
        ...
        if (!this.IsFieldUnderAttack(king.field.x, king.field.y, color))
        {
            ...
            foreach (Figure figure in figures)
            {
                ...
                foreach (Field field in figure.BeatFields)
                {
                    ...
                    if (figure.type == FigureTypes.Pow && ftt == null)
                    {
                        Figure f = null;
                        if (color == Color.black)
                        ...
                        if (color == Color.white)
                          f = this.GetFigureByXY(field.x, (sbyte)(field.y - 1));
                        if (f != null && f.type == FigureTypes.Pow && f.moveCount == this.moveCount)
                            ftt = f;
                    }
